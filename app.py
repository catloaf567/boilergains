...
try:
    from protein import load_foods_from_excel, suggest_meal, format_meal, expand_excluded_items
except Exception as e:
    load_foods_from_excel = None
    suggest_meal = None
    format_meal = None
    expand_excluded_items = None
    _import_error = str(e)
else:
    _import_error = None
...
def suggest():
    ...
    raw_exclusions = data.get('excluded_items') or []
    excluded_items = []
    if isinstance(raw_exclusions, list):
        for item in raw_exclusions:
            if isinstance(item, str) and item.strip():
                excluded_items.append(item.strip())
    elif raw_exclusions:
        excluded_items.append(str(raw_exclusions))
    exclusion_tokens = []
    if excluded_items:
        if expand_excluded_items:
            exclusion_tokens = expand_excluded_items(excluded_items)
        else:
            exclusion_tokens = [item.lower() for item in excluded_items]
...
def _recommended_goals(age: float, gender: str, height_cm: float, weight_kg: float,
                       activity_level: str, meals_per_day: int = 3) -> Tuple[float, float, float, float, float, float, float, float, float, float]:
    """Return per-meal and daily targets for calories, protein, carbs, fat, and fiber."""
    if meals_per_day <= 0:
    ...
    # baseline macronutrient distribution if demographic helper unavailable
    daily_carbs = (daily_calories * 0.5) / 4.0
    daily_fat = (daily_calories * 0.25) / 9.0
    daily_fiber = (daily_calories / 1000.0) * 14.0

    if calculate_nutrition_needs:
        activity_map = {
            'sedentary': 'sedentary',
        ...
        demographics_activity = activity_map.get((activity_level or 'sedentary').strip().lower(), 'sedentary')
        demographics_gender = gender_key if gender_key in ('male', 'female') else None
        try:
            if demographics_gender:
                profile = calculate_nutrition_needs(
                    age,
                    weight_kg,
                    height_cm,
                    demographics_gender,
            ...
            profile = None
        if profile:
            daily_carbs = float(profile.get('carbohydrates_g', daily_carbs))
            daily_fat = float(profile.get('fat_g', daily_fat))
            daily_fiber = float(profile.get('fiber_g', daily_fiber))

    per_meal_carbs = daily_carbs / meals_per_day
    per_meal_fat = daily_fat / meals_per_day
    per_meal_fiber = daily_fiber / meals_per_day
...
def _format_meal_response(meal: dict, foods: dict, protein_window: float = 10.0, max_options: int = 4) -> str:
...
def recommend():
    ...
    return jsonify(
        ...
        carb_goal=round(per_meal_carbs, 1),
        fat_goal=round(per_meal_fat, 1),
        fiber_goal=round(per_meal_fiber, 1),
        ...
        daily_fiber_goal=round(daily_fiber, 1),
        meals_per_day=meals_per_day,
    )
